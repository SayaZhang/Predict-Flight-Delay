...
def load_data():
    ...
    weather_to['weatherVecTo'] = weather_to['weatherVecTo'].apply(lambda x:  np.matrix([x]))
    
    # 机场特情
    sn = load_special_news()
   
    #weather_to['weatherVecTo'] = weather_to['weatherVecTo'].apply(lambda x: [float(i) for i in x[2:-1].split('  ')])
    #print weather_to.ix[0,'weatherVecTo']
    ...
    for index,row in sn.iterrows():
        ...
        data[((data[u'出发机场'] == row[u'特情机场']) | (data[u'到达机场'] == row[u'特情机场'])) & ((data[u'计划起飞时间'] > row[u'开始时间']) & (data[u'计划起飞时间'] < row[u'结束时间']))]['hasSpecialNews'] = 1
        #break
...
def pretreat(df):
    ...
    # 是否延误超过3小时 或 航班取消
    df['isMoreThan3'] = df['delay'].apply(lambda x: x>=3*3600 and True) | df[u'航班是否取消'].apply(lambda x: x == u'取消' and True)
    ...
    # time/1000
    df[u'计划起飞时间'] = df[u'计划起飞时间']/1000
    df[u'计划到达时间'] = df[u'计划到达时间']/1000
    ...
    for flight in df.groupby([u'航班编号', u'出发机场']):
        ...
        if len(flight[1])>1:              
            flight_feature.append([flight[0][0],flight[0][1],describe['mean'],describe['std'],describe['max'],describe['50%']])
        else: 
            flight_feature.append([flight[0][0],flight[0][1],describe['mean'],0,describe['max'],describe['50%']])
    df_flight_feature = pd.DataFrame(flight_feature,columns=[u'航班编号',u'出发机场',u'平均延误时间',u'延误时间标准差',u'最大延误时间',u'延误时间中位数'])
...
def model_cmd():
    ...
    for train, test in skf:
        ...
        log(res_path, "| Accuracy: "+str(acc)+" | Precision: "+str(pre)+" | Recall: "+str(recall)+" | F1: "+str(f1))
        count += 1
        print "--------------------------------"
        log(res_path, "--------------------------------")
